...
The recommended way to install FavoriteFiles is via [Package Control][package-control].  Package Control will install the correct branch on your system and keep it up to date.
...
3. Start typing `FavoriteFiles`; when you see it, select it.
...
!!! warning "Warning"
    This is not the recommended way to install FavoriteFiles for the average user.  Installing this way **will not** get automatically updated.

    If you are forking for a pull request, you should **just** clone FavoriteFiles and run Package Control's `Satisfy Dependency` command to get all the dependencies.

For those who want to install FavoriteFiles without package control, here are the steps.  It understood that some people for what ever reason will prefer manually and may even have legitimate reasons to do so.  When going this route, you will have to keep all the packages updated yourself.
...
2. Download and unpack or git clone the latest FavoriteFiles release and unpack as `FavoriteFiles`:

    - https://github.com/facelessuser/FavoriteFiles -> FavoriteFiles
...
2. Open a terminal and enter the following.  For dependencies replace the URL with the appropriate URL, and the appropriate folder to check it out to:
    ...
    git clone https://github.com/facelessuser/FavoriteFiles.git FavoriteFiles
    ```
